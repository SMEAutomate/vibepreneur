...
export function AnimatedRoadmapIteration() {
  const phase = useAnimationLoop(5);
          ...
          >
            ...
            <div className="space-y-3">
              {col.items.map((item, ii) => {
                const isTopCard = ci === 0 && ii === 0;
                return (
                  <motion.div
                    key={item.title}
                    animate={{
                      opacity: phase >= ci ? 1 : 0,
                      y: phase >= ci ? 0 : 12,
                      boxShadow:
                        isTopCard && phase >= 4
