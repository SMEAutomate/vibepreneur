...
export function AnimatedGrowthDashboard() {
  const phase = useAnimationLoop(8);
  ...
  return (
    ...
    >
      {/* 4 metrics: phases 0-3 */}
      <div className="grid gap-4 md:grid-cols-4">
        {metrics.map((m, i) => (
          <motion.div
            key={m.label}
            animate={show(phase, i)}
          ...
          <div className="mt-4">
            ...
            {sources.map((s) => (
              ...
              >
                ...
                <span className="text-right text-neutral-500">{s.signups}</span>
          ...
          <div className="mt-4 space-y-3">
            {experiments.map((e) => (
              ...
              >
                ...
                <p className="mt-1 text-[10px] text-neutral-500">{e.result}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {/* Growth Moves - phase 6 */}
      <motion.div
        animate={show(phase, 6)}
        transition={{ duration: 0.4, ease: EASE }}
        className="mt-6 rounded-2xl border border-brand-100 bg-brand-50/30 p-6"
      >
        <h2 className="text-sm font-semibold text-brand-800">
          Next 3 Growth Moves
        </h2>
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          {growthMoves.map((move, i) => (
            <div
              key={move}
