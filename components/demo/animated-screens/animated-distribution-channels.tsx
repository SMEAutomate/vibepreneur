...
const recSequence = ["LinkedIn organic", "Cold email", "Free audit tool"];

export function AnimatedDistributionChannels() {
  const phase = useAnimationLoop(9);
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        {channels.map((ch, i) => (
          <motion.div
            ...
            className="flex items-center gap-5 rounded-2xl border border-neutral-200 p-5"
          >
      ...
      {/* Recommended Sequence - phase 7 */}
      <motion.div
        animate={show(phase, 7)}
        transition={{ duration: 0.4, ease: EASE }}
        className="mt-8 rounded-2xl border border-brand-100 bg-brand-50/30 p-6"
      >
        <h2 className="text-sm font-semibold text-brand-800">
          Recommended Sequence
        </h2>
        <div className="mt-4 flex items-center gap-3">
          {recSequence.map((step, i) => (
